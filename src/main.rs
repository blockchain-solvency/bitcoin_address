...
use rand::{distributions::Uniform, Rng, seq::SliceRandom}; // 0.6.5
...
fn main() {
    ...
    let G = BaseGroup::new();
    let Y_1024: Vec<(primitive_types::U512,primitive_types::U512)> = secret_set.iter().map(|n| G.exponentiate(*n)).collect();
    println!("{:?} ", Y_1024);
